...
function day10_1(data: string) {
    ...
    for (let i = 0; i < grid.length; i++) {
        for (let j = 0; j < grid[i].length; j++) {
            ...
            if (element === 0) {
                ...
                let reachable: Map<number[], boolean> = new Map();
    ...
    function explore(row: number, col: number, height: number, visited: Map<number[], boolean>, reachable: Map<number[], boolean>) {
        ...
        if (visited.get(currentPos)) return;

        visited.set(currentPos, true);

        if (height === 9) {
            reachable.set(currentPos, true);
            return;
        }
